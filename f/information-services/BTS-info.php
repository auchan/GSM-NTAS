...
<div class="container">
    ...
    <ul class="nav nav-tabs">
        <li class="active"><a href="f/information-services/BTS-info.php">基站信息查询</a></li>
        <li><a href="f/information-services/CELL-info.php">小区信息查询</a></li>
    </ul>
	<br>
			...
			<div class="row">
				<div class="col-md-4">
					<div class="input-group">
						<!--input type="text" class="form-control" placeholder="请输入基站名称" id="inputval"-->
						<select id="select-btsname" class="selectpicker" data-width="100px">
							<option>请选择</option>
						</select>
...
$(document).ready(function (){
	$.getJSON(
	"Action/101GetAllBtsname.php",
	function (data) {	
		for (var i in data) {
			var option = $("<option></option>").html(data[i]).appendTo('#select-btsname');
		}
...
$(function () {
	...
	//var btsname = $("#inputval").val();
	var btsname = $('.selectpicker').selectpicker('val');
		if (btsname === "请选择") {
			 return 0;
		}
		$.getJSON(
		"Action/11QueryBTS.php",
		{btsname:btsname},
		function (data) {
			if (data.success) {
				...
				if (!data.rownum) {
						$('#inputval').popover('destroy');
						 $('#inputval').popover({content: "基站名: "+btsname+" 在数据库中不存在", 
						 placement:'bottom',
						 trigger:'manual'
						 });	

...
$sql = "exec traffic_hour $CellID, $date_s, $time_s, $date_e, $time_e";
...
while (odbc_fetch_row($rs)) {
	...
	$hoursTraffic = odbc_result($rs, 3);
	$hoursCallcongs = odbc_result($rs, 4);
	$hoursTrafficPerTCH = odbc_result($rs, 5);
	if (strlen($data) == 5) {
	...
	$tup = array("datetime"=>$datetime, "hoursTraffic"=>$hoursTraffic, "hoursCallcongs"=>$hoursCallcongs,
					"hoursTrafficPerTCH"=>$hoursTrafficPerTCH);
	$json["rows"][] = $tup;
	$rownum++;
}
$json["rownum"] = $rownum;
